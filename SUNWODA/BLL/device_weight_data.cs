...
{
    /// <summary>
    /// BLL for device_weight_data
    /// </summary>
    public partial class device_weight_data
    {
        private readonly DAL.device_weight_data _dal = new DAL.device_weight_data();
        ...
        public bool Add(Model.device_weight_data model) => _dal.Add(model);

        /// <summary>
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.device_weight_data model)
        {
            bool success = _dal.Update(model);
            if (success)
            {
                _memoryCache.Remove($"device_weight_dataModel-{model.id}");
            }
            return success;
        }
        ...
        public bool Delete(int id)
        {
            ...
            if (success)
            {
                _memoryCache.Remove($"device_weight_dataModel-{id}");
            }
            return success;
        }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.device_weight_data? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.device_weight_data? GetModelByCache(int id)
        {
            string cacheKey = $"device_weight_dataModel-{id}";
            if (!_memoryCache.TryGetValue(cacheKey, out Model.device_weight_data? model))
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_weight_data> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.device_weight_data> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.device_weight_data>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.device_weight_data model) => Task.Run(() => Add(model));

        public async Task<bool> UpdateAsync(Model.device_weight_data model)
        {
        ...
        public Task<Model.device_weight_data?> GetModelAsync(int id) =>
            Task.Run(() => GetModel(id));

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public async Task<Model.device_weight_data?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"device_weight_dataModel-{id}";
        ...
        public Task<List<Model.device_weight_data>> GetModelListAsync(string strWhere)
        {
