...
{
    /// <summary>
    /// BLL for device_andon_temp
    /// </summary>
    public partial class device_andon_temp
    {
        private readonly DAL.device_andon_temp _dal = new DAL.device_andon_temp();
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(Model.device_andon_temp model) => _dal.Add(model);

        /// <summary>
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.device_andon_temp model) => _dal.Update(model);
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.device_andon_temp? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.device_andon_temp? GetModelByCache(int id)
        {
            string cacheKey = $"device_andon_tempModel-{id}";

            if (!_memoryCache.TryGetValue(cacheKey, out Model.device_andon_temp? model))
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_andon_temp> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_andon_temp> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.device_andon_temp>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.device_andon_temp model) => Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.device_andon_temp model) =>
        ...
        public Task<Model.device_andon_temp?> GetModelAsync(int id) => Task.Run(() => GetModel(id));

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public async Task<Model.device_andon_temp?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"device_andon_tempModel-{id}";
        ...
        public Task<List<Model.device_andon_temp>> GetModelListAsync(string strWhere)
        {
