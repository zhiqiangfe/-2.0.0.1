...
namespace SUNWODA_SEVB.Data.BLL
{
    ...
    {
        private readonly DAL.device_tag _dal = new DAL.device_tag();
        ...
        public bool Add(Model.device_tag model) => _dal.Add(model);

        /// <summary>
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.device_tag model)
        {
            bool success = _dal.Update(model);
            if (success)
            {
                _memoryCache.Remove($"device_tagModel-{model.id}");
            }
            return success;
        }
        ...
        public bool Delete(int id)
        {
            ...
            if (success)
            {
                _memoryCache.Remove($"device_tagModel-{id}");
            }
            return success;
        }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.device_tag? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.device_tag? GetModelByCache(int id)
        {
            string cacheKey = $"device_tagModel-{id}";
            if (!_memoryCache.TryGetValue(cacheKey, out Model.device_tag? model))
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_tag> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.device_tag> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.device_tag>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.device_tag model) => Task.Run(() => Add(model));

        public async Task<bool> UpdateAsync(Model.device_tag model)
        {
        ...
        public Task<Model.device_tag?> GetModelAsync(int id) => Task.Run(() => GetModel(id));

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public async Task<Model.device_tag?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"device_tagModel-{id}";
        ...
        public Task<List<Model.device_tag>> GetModelListAsync(string strWhere)
        {
