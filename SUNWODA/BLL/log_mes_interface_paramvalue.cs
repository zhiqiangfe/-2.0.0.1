...
{
    /// <summary>
    /// BLL for log_mes_interface_paramvalue
    /// </summary>
    public partial class log_mes_interface_paramvalue
    {
        private readonly DAL.log_mes_interface_paramvalue _dal =
            new DAL.log_mes_interface_paramvalue();
        ...
        public bool Add(Model.log_mes_interface_paramvalue model) => _dal.Add(model);

        public bool Update(Model.log_mes_interface_paramvalue model) => _dal.Update(model);
        ...
        public Model.log_mes_interface_paramvalue? GetModel(int id) => _dal.GetModel(id);
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.log_mes_interface_paramvalue> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.log_mes_interface_paramvalue> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.log_mes_interface_paramvalue>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.log_mes_interface_paramvalue model) =>
            Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.log_mes_interface_paramvalue model) =>
        ...
        public Task<Model.log_mes_interface_paramvalue?> GetModelAsync(int id) =>
            Task.Run(() => GetModel(id));

        public Task<List<Model.log_mes_interface_paramvalue>> GetModelListAsync(string strWhere)
        {
