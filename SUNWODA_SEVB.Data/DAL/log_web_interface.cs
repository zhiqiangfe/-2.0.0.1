...
namespace SUNWODA_SEVB.Data.DAL
{
    public partial class log_web_interface
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from log_web_interface");
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(SUNWODA_SEVB.Data.Model.log_web_interface model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into log_web_interface(");
            strSql.Append(
                "logdate,method,input_json,output_json,start_time,end_time,consuming_time)"
            );
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(SUNWODA_SEVB.Data.Model.log_web_interface model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update log_web_interface set ");
        ...
        /// 得到一个对象实体
        /// </summary>
        public SUNWODA_SEVB.Data.Model.log_web_interface? GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(
                "select id,logdate,method,input_json,output_json,start_time,end_time,consuming_time from log_web_interface "
            );
            strSql.Append(" where id=@id");
            MySqlParameter[] parameters = { new MySqlParameter("@id", MySqlDbType.Int32) };
            parameters[0].Value = id;
        ...
        /// 得到一个对象实体
        /// </summary>
        public SUNWODA_SEVB.Data.Model.log_web_interface DataRowToModel(DataRow row)
        {
            SUNWODA_SEVB.Data.Model.log_web_interface model = new SUNWODA_SEVB.Data.Model.log_web_interface();
            if (row != null)
            {
        ...
        public DataSet GetList(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(
                "select id,logdate,method,input_json,output_json,start_time,end_time,consuming_time "
            );
            strSql.Append(" FROM log_web_interface ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from log_web_interface T ");
