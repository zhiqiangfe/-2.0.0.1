...
{
    /// <summary>
    /// BLL for alarm_record
    /// </summary>
    public partial class alarm_record
    {
        private readonly DAL.alarm_record _dal = new DAL.alarm_record();
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(Model.alarm_record model) => _dal.Add(model);

        /// <summary>
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.alarm_record model) => _dal.Update(model);
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.alarm_record? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.alarm_record? GetModelByCache(int id)
        {
            string cacheKey = $"alarm_recordModel-{id}";

            if (!_memoryCache.TryGetValue(cacheKey, out Model.alarm_record? model))
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.alarm_record> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.alarm_record> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.alarm_record>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.alarm_record model) => Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.alarm_record model) => Task.Run(() => Update(model));

        public Task<bool> DeleteAsync(int id) => Task.Run(() => Delete(id));

        public Task<Model.alarm_record?> GetModelAsync(int id) => Task.Run(() => GetModel(id));

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public async Task<Model.alarm_record?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"alarm_recordModel-{id}";
        ...
        public Task<List<Model.alarm_record>> GetModelListAsync(string strWhere)
        {
