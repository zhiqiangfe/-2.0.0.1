...
{
    /// <summary>
    /// BLL for log_operation
    /// </summary>
    public partial class log_operation
    {
        private readonly DAL.log_operation _dal = new DAL.log_operation();
        ...
        public bool Add(Model.log_operation model) => _dal.Add(model);

        /// <summary>
        ///
        /// </summary>
        public bool Update(Model.log_operation model) => _dal.Update(model);
        ...
        public Model.log_operation? GetModel(int id) => _dal.GetModel(id);
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.log_operation> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.log_operation> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.log_operation>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.log_operation model) => Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.log_operation model) => Task.Run(() => Update(model));
        ...
        public Task<Model.log_operation?> GetModelAsync(int id) => Task.Run(() => GetModel(id));

        public Task<List<Model.log_operation>> GetModelListAsync(string strWhere)
        {
