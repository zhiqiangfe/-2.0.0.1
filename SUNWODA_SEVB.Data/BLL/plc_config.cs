...
namespace SUNWODA_SEVB.Data.BLL
{
    ...
    {
        private readonly DAL.plc_config _dal ;
        private readonly IMemoryCache _memoryCache;
        private readonly int _cacheDurationMinutes;

        public plc_config(DAL.plc_config dal,IMemoryCache memoryCache,IConfiguration configuration)
        {
            _dal = dal;
        ...
        public bool Add(Model.plc_config model)
        {
            return _dal.Add(model);
        }

        public bool Update(Model.plc_config model)
        {
            bool success = _dal.Update(model);
            if (success)
            {
                _memoryCache.Remove($"plc_configModel-{model.id}");
            }
            return success;
        }
        ...
        public bool Delete(int id)
        {
            ...
            if (success)
            {
                _memoryCache.Remove($"plc_configModel-{id}");
            }
            return success;
        }
        ...
        public Model.plc_config? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.plc_config? GetModelByCache(int id)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.plc_config> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.plc_config> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.plc_config>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.plc_config model) => Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.plc_config model) => Task.Run(() => Update(model));
        ...
        ///
        /// </summary>
        public Task<Model.plc_config?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"plc_configModel-{id}";
        ...
        public Task<List<Model.plc_config>> GetModelListAsync(string strWhere)
        {
