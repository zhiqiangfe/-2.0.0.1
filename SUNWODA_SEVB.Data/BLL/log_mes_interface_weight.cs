...
{
    /// <summary>
    /// BLL for log_mes_interface_weight
    /// </summary>
    public partial class log_mes_interface_weight
    {
        private readonly DAL.log_mes_interface_weight _dal = new DAL.log_mes_interface_weight();
        ...
        ///
        /// </summary>
        public bool Add(Model.log_mes_interface_weight model) => _dal.Add(model);

        /// <summary>
        ///
        /// </summary>
        public bool Update(Model.log_mes_interface_weight model) => _dal.Update(model);
        ...
        public Model.log_mes_interface_weight? GetModel(int id) => _dal.GetModel(id);
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.log_mes_interface_weight> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.log_mes_interface_weight> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.log_mes_interface_weight>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.log_mes_interface_weight model) =>
            Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.log_mes_interface_weight model) =>
        ...
        public Task<Model.log_mes_interface_weight?> GetModelAsync(int id) =>
            Task.Run(() => GetModel(id));

        public Task<List<Model.log_mes_interface_weight>> GetModelListAsync(string strWhere)
        {
