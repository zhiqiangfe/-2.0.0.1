...
{
    /// <summary>
    /// BLL for log_http_interface
    /// </summary>
    public partial class log_http_interface
    {

        private readonly DAL.log_http_interface _dal ;

        public log_http_interface(DAL.log_http_interface dal) { _dal = dal; }
        ...
        public bool Add(Model.log_http_interface model) => _dal.Add(model);

        /// <summary>
        ///
        /// </summary>
        public bool Update(Model.log_http_interface model) => _dal.Update(model);
        ...
        public Model.log_http_interface? GetModel(int id) => _dal.GetModel(id);
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.log_http_interface> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.log_http_interface> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.log_http_interface>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.log_http_interface model) => Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.log_http_interface model) =>
        ...
        public Task<Model.log_http_interface?> GetModelAsync(int id) =>
            Task.Run(() => GetModel(id));

        public Task<List<Model.log_http_interface>> GetModelListAsync(string strWhere)
        {
