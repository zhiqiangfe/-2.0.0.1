...
{
    /// <summary>
    /// BLL for device_inputoutput_config
    /// </summary>
    public partial class device_inputoutput_config
    {
        private readonly DAL.device_inputoutput_config _dal = new DAL.device_inputoutput_config();
        ...
        public bool Add(Model.device_inputoutput_config model) => _dal.Add(model);

        /// <summary>
        /// 更新一条数据。
        /// </summary>
        public bool Update(Model.device_inputoutput_config model)
        {
            bool success = _dal.Update(model);
            if (success)
            {
                _memoryCache.Remove($"device_inputoutput_configModel-{model.id}");
            }
            return success;
        }
        ...
        public bool Delete(int id)
        {
            ...
            if (success)
            {
                _memoryCache.Remove($"device_inputoutput_configModel-{id}");
            }
            return success;
        }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.device_inputoutput_config? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.device_inputoutput_config? GetModelByCache(int id)
        {
            string cacheKey = $"device_inputoutput_configModel-{id}";
            if (!_memoryCache.TryGetValue(cacheKey, out Model.device_inputoutput_config? model))
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_inputoutput_config> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_inputoutput_config> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.device_inputoutput_config>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.device_inputoutput_config model) =>
            Task.Run(() => Add(model));

        public async Task<bool> UpdateAsync(Model.device_inputoutput_config model)
        {
        ...
        public Task<Model.device_inputoutput_config?> GetModelAsync(int id) =>
            Task.Run(() => GetModel(id));

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public async Task<Model.device_inputoutput_config?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"device_inputoutput_configModel-{id}";
        ...
        public Task<List<Model.device_inputoutput_config>> GetModelListAsync(string strWhere)
        {
