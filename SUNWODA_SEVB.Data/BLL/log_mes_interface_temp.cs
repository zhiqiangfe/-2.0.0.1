...
{
    /// <summary>
    /// BLL for log_mes_interface_temp
    /// </summary>
    public partial class log_mes_interface_temp
    {
        private readonly DAL.log_mes_interface_temp _dal = new DAL.log_mes_interface_temp();
        ...
        ///
        /// </summary>
        public bool Add(Model.log_mes_interface_temp model) => _dal.Add(model);

        /// <summary>
        ///
        /// </summary>
        public bool Update(Model.log_mes_interface_temp model) => _dal.Update(model);
        ...
        ///
        /// </summary>
        public Model.log_mes_interface_temp? GetModel(int id) => _dal.GetModel(id);
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.log_mes_interface_temp> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.log_mes_interface_temp> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.log_mes_interface_temp>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.log_mes_interface_temp model) =>
            Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.log_mes_interface_temp model) =>
        ...
        public Task<Model.log_mes_interface_temp?> GetModelAsync(int id) =>
            Task.Run(() => GetModel(id));

        public Task<List<Model.log_mes_interface_temp>> GetModelListAsync(string strWhere)
        {
