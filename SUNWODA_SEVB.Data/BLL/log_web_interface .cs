...
namespace SUNWODA_SEVB.Data.BLL
{
    ...
    public partial class log_web_interface
    {
        ...
        public bool Add(Model.log_web_interface model) => _dal.Add(model);

        /// <summary>
        ///
        /// </summary>
        public bool Update(Model.log_web_interface model) => _dal.Update(model);
        ...
        public Model.log_web_interface? GetModel(int id) => _dal.GetModel(id);
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.log_web_interface> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.log_web_interface> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.log_web_interface>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.log_web_interface model) => Task.Run(() => Add(model));

        public Task<bool> UpdateAsync(Model.log_web_interface model) =>
        ...
        public Task<Model.log_web_interface?> GetModelAsync(int id) => Task.Run(() => GetModel(id));

        public Task<List<Model.log_web_interface>> GetModelListAsync(string strWhere)
        {
