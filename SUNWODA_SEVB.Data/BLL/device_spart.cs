...
{
    /// <summary>
    /// BLL for device_spart
    /// </summary>
    public partial class device_spart
    {
        private readonly DAL.device_spart _dal = new DAL.device_spart();
        ...
        public bool Add(Model.device_spart model) => _dal.Add(model);

        /// <summary>
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.device_spart model)
        {
            bool success = _dal.Update(model);
            if (success)
            {
                _memoryCache.Remove($"device_spartModel-{model.id}");
            }
            return success;
        }
        ...
        public bool Delete(int id)
        {
            ...
            if (success)
            {
                _memoryCache.Remove($"device_spartModel-{id}");
            }
            return success;
        }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.device_spart? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.device_spart? GetModelByCache(int id)
        {
            string cacheKey = $"device_spartModel-{id}";
            if (!_memoryCache.TryGetValue(cacheKey, out Model.device_spart? model))
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_spart> GetModelList(string strWhere)
        {
            DataSet ds = _dal.GetList(strWhere);
            return DataTableToList(ds.Tables[0]);
        }

        public List<Model.device_spart> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.device_spart>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.device_spart model) => Task.Run(() => Add(model));

        public async Task<bool> UpdateAsync(Model.device_spart model)
        {
        ...
        public Task<Model.device_spart?> GetModelAsync(int id) => Task.Run(() => GetModel(id));

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public async Task<Model.device_spart?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"device_spartModel-{id}";
        ...
        public Task<List<Model.device_spart>> GetModelListAsync(string strWhere)
        {
