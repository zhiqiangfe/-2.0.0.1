...
{
    /// <summary>
    /// BLL for device_materialinfo_record
    /// </summary>
    public partial class device_materialinfo_record
    {
        private readonly DAL.device_materialinfo_record _dal = new DAL.device_materialinfo_record();
        ...
        public bool Add(Model.device_materialinfo_record model) => _dal.Add(model);

        /// <summary>
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.device_materialinfo_record model)
        {
            bool success = _dal.Update(model);
            if (success)
            {
                _memoryCache.Remove($"device_materialinfo_recordModel-{model.id}");
            }
            return success;
        }
        ...
        public bool Delete(int id)
        {
            ...
            if (success)
            {
                _memoryCache.Remove($"device_materialinfo_recordModel-{id}");
            }
            return success;
        }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.device_materialinfo_record? GetModel(int id) => _dal.GetModel(id);

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Model.device_materialinfo_record? GetModelByCache(int id)
        {
            string cacheKey = $"device_materialinfo_recordModel-{id}";
            if (!_memoryCache.TryGetValue(cacheKey, out Model.device_materialinfo_record? model))
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_materialinfo_record> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.device_materialinfo_record> DataTableToList(DataTable dt)
        {
            var modelList = new List<Model.device_materialinfo_record>();
            if (dt.Rows.Count > 0)
            {
        ...
        public Task<bool> AddAsync(Model.device_materialinfo_record model) =>
            Task.Run(() => Add(model));

        public async Task<bool> UpdateAsync(Model.device_materialinfo_record model)
        {
        ...
        public Task<Model.device_materialinfo_record?> GetModelAsync(int id) =>
            Task.Run(() => GetModel(id));

        /// <summary>
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public async Task<Model.device_materialinfo_record?> GetModelByCacheAsync(int id)
        {
            string cacheKey = $"device_materialinfo_recordModel-{id}";
        ...
        public Task<List<Model.device_materialinfo_record>> GetModelListAsync(string strWhere)
        {
